...
$Id: plush.py 51524 2007-04-12 08:25:31Z bdelbosc $
"""
import sys
import os
...
Using PyLucene %s and Lucene (TODO version).
  ...
  Type: \open [INDEX_PATH]    to open a lucene store
    ...
    def do_term(self, message):
        ...
        items, count = self.getTermFreqs(field, self.limit, pattern)
        if not items:
            print "No match."
            return
        ...
        print "---------- ----"
        print "Number of terms for field '%s': %d" % (field and field or '*',
                                                      count)
